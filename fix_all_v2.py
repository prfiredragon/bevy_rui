...
def process(file):
    ...
    # --- VisualBox fix for shrinking backgrounds ---
    content = re.sub(r'ImageNode::solid_color\((.*?)\)', r'ImageNode { visual_box: bevy::ui::widget::VisualBox::PaddingBox, ..ImageNode::solid_color(\1) }', content)
    
    # Fix apply_rui_theme to also set VisualBox to PaddingBox for themed elements so ninepatch stretches properly
    if "theme.rs" in file:
    ...
    # Fix ImageNode::default() for backgrounds where we want PaddingBox.
    # Note: icons also use ImageNode::default() but they don't have padding so setting PaddingBox is harmless or we can just replace specifically
